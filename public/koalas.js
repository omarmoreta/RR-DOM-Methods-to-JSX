const koalas = [
  {
    name: "Basil",
    description: `This koala is loving towards me, and will always come when called. She is distrustful of strangers, and dislikes other koalas.`,
    imageURL: "./assets/basil.jpeg",
  },
  {
    name: "Sage",
    description:
      "This chill koala is friendly toward me, and will follow me as I explore. She seems to let anyone get near her, and will happily play with other koalas.",
    imageURL: "./assets/sage.jpeg",
  },
  {
    name: "Pine Nut",
    description:
      "This inquisitive koala is cautious of me, but will sometimes come when called.",
    imageURL: "./assets/pine-nut.jpeg",
  },
  {
    name: "Padfoot",
    description:
      "This koala seems to be the reluctant leader of the others. They mostly just sleep.",
    imageURL: "./assets/padfoot.jpeg",
  },
  {
    name: "Bob",
    description: "I've only just met Bob, but we're off to a good start.",
    imageURL: "./assets/bob.jpeg",
  },
];

exports = koalas;
